...
class heterograph_full_info(ObservationTransform):
    def _apply_transform(self, obs: torch.Tensor) -> NonTensorData:
        data = HeteroData()
        data['torso'].x = torch.tensor([[obs[0], obs[1], obs[2], obs[3], obs[4], obs[5], obs[6], obs[7], obs[8], obs[9], 
                                        obs[10], obs[11], obs[12], obs[13], obs[14], obs[15], obs[16], obs[17], obs[18], obs[19],
                                        obs[20], obs[21], obs[22], obs[23], obs[24], obs[25], obs[26],                                        
                                        1, 0, 0, 0, 0, 0, 0, 0, 0],]
                                        , dtype=torch.float)
        data['joint'].x = torch.tensor([[obs[0], obs[1], obs[2], obs[3], obs[4], obs[5], obs[6], obs[7], obs[8], obs[9], 
                                        obs[10], obs[11], obs[12], obs[13], obs[14], obs[15], obs[16], obs[17], obs[18], obs[19],
                                        obs[20], obs[21], obs[22], obs[23], obs[24], obs[25], obs[26],                                        
                                        0, 1, 0, 0, 0, 0, 0, 0, 0],
...
class heterograph_old(ObservationTransform):
    def _apply_transform(self, obs: torch.Tensor) -> NonTensorData:
        data = HeteroData()
        data['torso'].x = torch.tensor([[obs[0], obs[1], obs[2], obs[3], obs[4], obs[13], obs[14], obs[15], obs[16], obs[17], obs[18]],
                                ], dtype=torch.float)
...
class full_contact(ObservationTransform):
    def _apply_transform(self, obs: torch.Tensor) -> NonTensorData:
        data = HeteroData()
        data['torso'].x = torch.cat([
            torch.as_tensor(obs[:111], dtype=torch.float).unsqueeze(0),
            torch.tensor([[0, 0, 0, 0, 0, 0, 0, 0, 1]], dtype=torch.float)
        ], dim=1)

        # Efficient joint nodes (8 nodes, each with 111 obs + 9 one-hot)
        obs_tensor = torch.as_tensor(obs[:111], dtype=torch.float).unsqueeze(0).repeat(8, 1)  # [8, 111]
        one_hot = torch.eye(9, dtype=torch.float)[:8]  # [8, 9]
        data['joint'].x = torch.cat([obs_tensor, one_hot], dim=1)  # [8, 120]
